...
exports.default = {
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'add') {
            try {
                ...
                const { metadata, imageUrl, isPromotion } = await (0, characterUtils_1.getMetadata)(card, personalCharacterName || '', interaction.user.id);
                const { name, description, personality, scenario, first_mes, mes_example } = metadata;
                if (card) {
                    const exists = await (0, characterUtils_1.characterExists)(name);
                    if (exists) {
                ...
                if (isPromotion) {
                    await (0, characterUtils_1.promoteCharacterToGlobal)(name, interaction.user.id, imageUrl);
                }
                else {
                    await db_1.default.query(`INSERT INTO characters 
                        ...
                        VALUES (NULL, $1, $2, $3, $4, $5, $6, $7)`, [name, description, personality, scenario, first_mes, mes_example, imageUrl]);
                }
                await interaction.editReply(isPromotion
            ...
            try {
                const deleted = await db_1.default.query('DELETE FROM characters WHERE user_id IS NULL AND character_name = $1', [name]);
                if (deleted.rowCount === 0) {
                    return await interaction.editReply(`❌ No global character named **${name}** was found.`);
                }
                const restored = await (0, characterUtils_1.restoreArchivedCharacter)(name || '');
                return await interaction.editReply(restored
            ...
            try {
                const { global: list } = await (0, characterUtils_1.getCharacterLists)(interaction.user.id);
    ...
    async autocomplete(interaction) {
        ...
        if (subcommand === 'delete') {
            return await (0, autocomplete_1.autocompleteGlobalCharacters)(interaction);
        }
